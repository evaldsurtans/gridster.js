...
;( function($, window, document, undefined) {
        ...
        fn.update_sizes = function(width, height, marginX, marginY) {
        ...
        fn.click_resize_handle = function(event) {
            ...
            for (var i = 0; i < this.options.resize_variations.length; i++) {
                ...
                if (parseInt($li.attr('data-sizex')) === variation.sizex && parseInt($li.attr('data-sizey')) === variation.sizey) {
                    i++;
                    ...
                    this.$el.trigger('gridster:resize_handle');
        ...
        fn.add_widget = function(html, size_x, size_y, col, row) {
            ...
            clearTimeout(self.timeotThrottleAddWidget2);
            self.timeotThrottleAddWidget2 = setTimeout(function() {
                self.timeotThrottleAddWidget2 = 0;
                self.set_dom_grid_height();
            }, 600);
         ...
         * @param {Boolean} execute at once
         * @return {Class} Returns the instance of the Gridster Class.
         */
        fn.remove_widget = function(el, silent, callback, noTimeout) {
            ...
            if (noTimeout) {
                this.remove_widget_callback($nexts, $el, silent, wgd, callback);
            } else {
                setTimeout($.proxy(this.remove_widget_callback, this), 100, $nexts, $el, silent, wgd, callback);
            }
        };

        /*
         * Callback to remove_widget
         */
        fn.remove_widget_callback = function($nexts, $el, silent, wgd, callback) {
            $el.remove();
            ...
            var self = this;
            clearTimeout(this.timeoutThrottleRemoveCallback);
            this.timeoutThrottleRemoveCallback = setTimeout(function() {
                self.timeoutThrottleRemoveCallback = 0;
                self.set_dom_grid_height();
            }, 600);
        ...
        fn.register_widget = function($el) {
            ...
            var wgd = {
                'col' : parseFloat($el.attr('data-col')),
                'row' : parseFloat($el.attr('data-row')),
                'size_x' : parseFloat($el.attr('data-sizex')),
                'size_y' : parseFloat($el.attr('data-sizey')),
                'el' : $el
            };
            ...
            if (swap && this.can_placeholder_be_set(to_col, to_row, player_size_x, player_size_y)) {
                for (var key in this.w_queue) {
                    ...
                    if (this.w_queue[key] != "full") {
                        this.new_move_widget_to_legacy(this.w_queue[key], col, row);
                    }
                }
        ...
        fn.is_empty = function(col, row) {
            if ( typeof this.gridmap[col] !== 'undefined' && ( typeof this.gridmap[col][row] === 'undefined' || this.gridmap[col][row] === undefined || this.gridmap[col][row] === false)) {
                return true;
            }
            return false;
        };
        ...
        //Must call this before changing positions
        //Clears out all previous positions
        fn.clear_grid_data = function() {
            var self = this;
            this.$widgets.each(function() {
                var widget_grid_data = $(this).coords().grid;
                if (widget_grid_data) {
                    self.remove_from_gridmap(widget_grid_data);
                }
            });

            for (var i = 0; i < page.base.gridster.gridmap.length; i++) {
                if (page.base.gridster.gridmap[i]) {
                    for (var j = 0; j < page.base.gridster.gridmap[i].length; j++) {
                        page.base.gridster.gridmap[i][j] = false;
                    }
                }
            }
        };
        
        fn.new_move_widget_to_legacy = function($widget, col, row) {
           var self = this;
        ...
        fn.new_move_widget_to = function($widget, col, row, clearData) {
            var self = this;
            var widget_grid_data = $widget.coords().grid;
        ...
        fn.move_widget_up = function($widget, y_units) {
            ...
            var actual_row = Math.floor(el_grid_data.row);
            var moved = [];
            var can_go_up = true;
            y_units || ( y_units = 1);
        ...
        fn.widgets_below = function($el) {
            ...
            //if (!el_grid_data || !el_grid_data.row)
            //    return $([]);
        ...
        fn._traversing_widgets = function(type, direction, col, row, callback) {
            ...
            var methods = {
                ...
                'for_each/below' : function() {
                    for ( trow = row + 1, max = ga[col].length; trow < max; trow += 0.5) {
        ...
        fn.set_dom_grid_height = function() {
            ...
            this.$widgets.each(function() {
                var $li = $(this);
                var posY = parseFloat($li.attr('data-row')) + parseFloat($li.attr('data-sizey'));
                if(maxPosY < posY) {
                    maxPosY = posY;
                    $liMaxPosY = $li;
                }
            ...
            var blockHeight = this.options.widget_base_dimensions[1] + this.options.widget_margins[1] * 2;
            var newHeight = ( parseFloat($liMaxPosY.attr('data-row')) - 1 + parseFloat($liMaxPosY.attr('data-sizey')) ) * blockHeight;
        ...
        fn.generate_stylesheet = function(opts) {
            ...
            for ( i = opts.cols; i >= 0; i--) {
                ...
                styles += (opts.namespace + ' [data-col="' + (i + 1) + '.5"] { left:' + (((i + 0.5) * opts.widget_base_dimensions[0]) + ((i + 0.5) * opts.widget_margins[0]) + (((i + 0.5) + 1) * opts.widget_margins[0])) + 'px;} ');
            }
            ...
            for ( i = opts.rows; i >= 0; i--) {
                ...
                styles += (opts.namespace + ' [data-row="' + (i + 1) + '.5"] { top:' + (((i + 0.5) * opts.widget_base_dimensions[1]) + ((i + 0.5) * opts.widget_margins[1]) + (((i + 0.5) + 1) * opts.widget_margins[1]) ) + 'px;} ');
            }

            for (var y = 1; y <= opts.rows; y++) {
                styles += (opts.namespace + ' [data-sizey="' + y + '"] { height:' + (y * opts.widget_base_dimensions[1] + (y - 1) * (opts.widget_margins[1] * 2)) + 'px;}');

                styles += (opts.namespace + ' [data-sizey="' + y + '.5"] { height:' + ((y + 0.5) * opts.widget_base_dimensions[1] + ((y + 0.5) - 1) * (opts.widget_margins[1] * 2)) + 'px;}');
            }

            for (var x = 1; x <= max_size_x; x++) {
                styles += (opts.namespace + ' [data-sizex="' + x + '"] { width:' + (x * opts.widget_base_dimensions[0] + (x - 1) * (opts.widget_margins[0] * 2)) + 'px;}');

                styles += (opts.namespace + ' [data-sizex="' + x + '.5"] { width:' + ((x + 0.5) * opts.widget_base_dimensions[0] + ((x + 0.5) - 1) * (opts.widget_margins[0] * 2)) + 'px;}');
            }
